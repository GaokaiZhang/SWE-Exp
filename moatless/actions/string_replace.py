...
def smart_normalize_escapes(text: str, target_text: str = None) -> str:
    ...
    if target_text:
        ...
        # 检测策略：如果用户输入包含字面量转义，而文件包含对应的实际字符
        for literal_escape, actual_char in escape_map.items():
            if literal_escape in result and actual_char in target_text:
                result = result.replace(literal_escape, actual_char)
...
def smart_fix_old_str(old_str: str, file_content: str) -> str:
    """
    智能修复用户输入的 old_str，使其能够匹配文件内容
    
    主要处理：
    1. 不完整的正则表达式字符串 - 补全缺失的 )\b"
    2. 转义字符问题 - 修复 \x08 为 \b
    """
    import re
    
    logger.info(f"=====smart_fix_old_str input====\n{old_str}")
    
    # 简单直接的修复策略
    fixed_old_str = old_str
    
    # 策略1：检测并修复不完整的正则表达式
    # 模式1: rf"#\s*({notes}|{self.config.notes_rgx}" -> rf"#\s*({notes}|{self.config.notes_rgx})\b"
    if 'rf"#\\s*(' in fixed_old_str and 'notes_rgx}"' in fixed_old_str:
        # 如果包含 notes_rgx}" 但不包含 )\b"，则补全
        if 'notes_rgx}"' in fixed_old_str and ')\\b"' not in fixed_old_str:
            fixed_old_str = fixed_old_str.replace('notes_rgx}"', 'notes_rgx})\\b"')
            logger.info("Applied fix: notes_rgx}\" -> notes_rgx})\\b\"")
    
    # 模式2: rf"#\s*({notes}" -> rf"#\s*({notes})\b"
    elif 'rf"#\\s*(' in fixed_old_str and 'notes}"' in fixed_old_str:
        if 'notes}"' in fixed_old_str and ')\\b"' not in fixed_old_str:
            fixed_old_str = fixed_old_str.replace('notes}"', 'notes})\\b"')
            logger.info("Applied fix: notes}\" -> notes})\\b\"")
    
    # 策略2：更通用的括号补全
    # 查找所有 rf"#\s*({...}" 模式但缺少 )\b" 的情况
    lines = fixed_old_str.split('\n')
    fixed_lines = []
    ...
    for line in lines:
        ...
        # 检测包含 rf"#\s*( 但缺少 )\b" 的行
        if 'rf"#\\s*(' in line and line.count('(') > line.count(')'):
            # 如果行以 }" 结尾，替换为 })\b"
            if line.strip().endswith('}"'):
                line = line[:-2] + '})\\b"'
                logger.info(f"Applied bracket fix: '{original_line}' -> '{line}'")
            # 如果行以 }" 结尾但没有引号，添加 )\b"
    ...
    # 策略3：转义字符修复
    if '\\x08' in fixed_old_str:
        fixed_old_str = fixed_old_str.replace('\\x08', '\\b')
        logger.info("Applied escape fix: \\x08 -> \\b")
    
    logger.info(f"=====smart_fix_old_str output====\n{fixed_old_str}")
    
    return fixed_old_str
...
class StringReplace(Action, CodeActionValueMixin, CodeModificationMixin):
    ...
    def execute(
        ...
        # logger.info(f'=====old_str====\n{old_str}')
        # logger.info(f'=====original old_str====\n{args.old_str}')
        # logger.info(f'=====new_str====\n{new_str}')
        # logger.info(f'=====original new_str====\n{args.new_str}')
        
        # old_str = smart_fix_old_str(old_str, file_content)
        # logger.info(f'=====fixed old_str====\n{old_str}')
    ...
    while start_idx < len(content_lines):
        ...
        for window_size in range(1, min(50, len(content_lines) - start_idx + 1)):
            ...
            if old_str_normalized in window_normalized:
                ...
                differences = []
                newline = "\n"
                if window.count(newline) != old_str.count(newline):
                    window_lines = window.count(newline) + 1
                    old_str_lines = old_str.count(newline) + 1
                    differences.append(
                        f"Line break count differs: found {window_lines} lines, "
                        f"expected {old_str_lines} lines"
                    )
